...
def seed_falkordb():
    """Seed FalkorDB with test data."""
    
    # Connect to FalkorDB
    print(f"Connecting to FalkorDB at {FALKORDB_HOST}:{FALKORDB_PORT}...")
    graph = Graph(host=FALKORDB_HOST, port=FALKORDB_PORT, graph_name=GRAPH_NAME)
    ...
    for c in customers:
        graph.execute(
            "CREATE (c:Customer {id: $id, name: $name, email: $email, phone: $phone, type: $type, created_at: datetime()})",
            c
        )
    ...
    for m in mechanics:
        graph.execute(
            "CREATE (m:Mechanic {id: $id, name: $name, email: $email, phone: $phone, specialties: $specialties, license_number: $license_number, certifications: $certifications, availability: $availability, created_at: datetime()})",
            m
        )
    ...
    for a in aircraft:
        graph.execute(
            "CREATE (a:Aircraft {id: $id, tail_number: $tail_number, model: $model, year: $year, status: $status, location: $location, owner_id: $owner_id, created_at: datetime()})",
            a
        )
    ...
    for j in jobs:
        graph.execute(
            "CREATE (j:Job {id: $id, title: $title, description: $description, status: $status, priority: $priority, scheduled_date: $scheduled_date, completed_date: $completed_date, created_at: datetime()})",
            j
        )
    ...
    for a in aircraft:
        graph.execute(
            "MATCH (c:Customer {id: $owner_id}), (a:Aircraft {id: $id}) CREATE (c)-[:OWNS]->(a)",
            {"id": a["id"], "owner_id": a["owner_id"]}
        )
    ...
    for j in jobs:
        if "mechanic_id" in j:
            graph.execute(
                "MATCH (m:Mechanic {id: $mechanic_id}), (j:Job {id: $id}) CREATE (m)-[:ASSIGNED]->(j)",
                {"id": j["id"], "mechanic_id": j["mechanic_id"]}
            )
    print(f"Created ASSIGNED relationships")
    
    # ===== CREATE TENANT NODE =====
    graph.execute(
        "CREATE (t:Tenant {id: $tenant_id, name: 'SkyMechanics', created_at: datetime()})",
        {"tenant_id": TENANT_ID}
    )
    ...
    for c in customers:
        graph.execute(
            "MATCH (t:Tenant {id: $tenant_id}), (c:Customer {id: $id}) CREATE (c)-[:BELONGS_TO]->(t)",
            {"tenant_id": TENANT_ID, "id": c["id"]}
        )
    
    for m in mechanics:
        graph.execute(
            "MATCH (t:Tenant {id: $tenant_id}), (m:Mechanic {id: $id}) CREATE (m)-[:BELONGS_TO]->(t)",
            {"tenant_id": TENANT_ID, "id": m["id"]}
        )
    
    for a in aircraft:
        graph.execute(
            "MATCH (t:Tenant {id: $tenant_id}), (a:Aircraft {id: $id}) CREATE (a)-[:BELONGS_TO]->(t)",
            {"tenant_id": TENANT_ID, "id": a["id"]}
        )
    
    for j in jobs:
        graph.execute(
            "MATCH (t:Tenant {id: $tenant_id}), (j:Job {id: $id}) CREATE (j)-[:BELONGS_TO]->(t)",
            {"tenant_id": TENANT_ID, "id": j["id"]}
        )
